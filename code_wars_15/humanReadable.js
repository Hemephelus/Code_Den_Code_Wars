...
function solution1(seconds) {
  ...
  let intHoursToSecs = intHours * 3600;
  let reminderSeconds = seconds - intHoursToSecs;

  let mins = reminderSeconds / 60;
  let intMins = Math.floor(mins);
  let intMinsToSecs = intMins * 60;
  let reminder_seconds = reminderSeconds - intMinsToSecs;

  console.log(intHours);
  console.log(intMins);
  console.log(reminder_seconds);

  return `${intHours.toString().padStart(2, 0)}:${intMins
    .toString()
    .padStart(2, 0)}:${reminder_seconds.toString().padStart(2, '0')}`;
}
